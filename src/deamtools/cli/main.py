...
def _add_bam2fragment_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "C->T positions, G->A positions [, barcode]). The 'count' column gives\n"
            "the number of reads/pairs producing that exact signature. The last\n"
            "two columns are '|'-separated lists of 0-based reference positions,\n"
            "'.' when empty:\n"
            "\n"
            "  C->T  a reference C read as T -- the TOP strand was deaminated\n"
            "  G->A  a reference G read as A -- the BOTTOM strand was deaminated\n"
            "\n"
            "Both are called regardless of read orientation. Which strand carries\n"
            "an edit is a property of the molecule, not of the record observing\n"
            "it: the BAM stores every read in reference orientation, and library\n"
            "prep fixes the deaminated U into a real T:A pair, so both patterns\n"
            "are visible on reads of either orientation. A dsDNA deaminase edits\n"
            "both strands, so one fragment routinely carries both.\n"
            "\n"
            ...
            "BAMs each read is treated as a single-end fragment. Where the mates\n"
            "overlap a position is reported once, the higher base quality winning\n"
            "a disagreement."
        ),
        epilog=(
            "examples:\n"
...
def _add_qc_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "    editable C/G opportunities) and edits-per-fragment\n"
            "    distribution. Mates are merged first, so a position both\n"
            "    mates cover counts once.\n"
            ...
            "    (paired-end only; the layout is detected from the BAM, and a\n"
            "    single-end library gets no pair metrics rather than zeros).\n"
