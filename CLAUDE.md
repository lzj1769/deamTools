...
- **`qc`.** `--n_reads` subsamples: the fraction is `n_reads / total` with the total taken from the BAM index (no extra scan), each read kept independently with that probability (uniform across the genome, unlike truncating a coordinate-sorted file), seeded from the chromosome name via `random.Random(str)` so reruns match — the builtin `hash()` is salted per process and would not. Rates stay unbiased; reported counts are counts *of the sample*, and `metrics["sampling"]` records the fraction. TSS enrichment is deliberately not subsampled. Single pass over the BAM accumulates: read stats, editing rate, per-read edit-rate distribution, trinucleotide context bias, fragment-length distribution, optional TSS enrichment, and a **deaminase motif PWM** (window around each edit, centre excluded, G-edits reverse-complemented). Outputs JSON + a self-contained HTML report that embeds a matplotlib summary figure and a `logomaker` motif logo (both base64, `Agg` backend).
